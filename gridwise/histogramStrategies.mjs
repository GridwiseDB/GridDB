...
export function radixBinCPU(inputValue, shift, radix) {
    const digit = (inputValue >>> shift) & (radix - 1);
    return digit;
}
...
function generateRadixWGSL(radix, datatype, numPasses, shift = 0) {
    const bitsPerPass = Math.log2(radix);
    const mask = radix - 1;
    
    if (numPasses > 1) {
        return `
fn bin_value(value: ${datatype}, passIndex: u32) -> u32 {
    let shift_amount = passIndex * ${bitsPerPass}u;
    let digit = (u32(value) >> shift_amount) & ${mask}u;
    return digit;
}
`;
    } else {
        return `
fn bin_value(value: ${datatype}) -> u32 {
    let digit = (u32(value) >> ${shift}u) & ${mask}u;
    return digit;
}
`;
    }
...
function validateRadixBins(bins, numPasses) {
    if (!bins.radix || bins.radix <= 0) {
        throw new Error(
            `HistogramStrategy: bins.radix must be positive integer, got ${bins.radix}`
        );
    }
    
    if (!Number.isInteger(Math.log2(bins.radix))) {
        throw new Error(
            `HistogramStrategy: bins.radix must be power of 2, got ${bins.radix}`
        );
    }
    
    if (numPasses > 1) {
        const totalBins = bins.radix * numPasses;
        const maxSharedMemoryBins = 4096;
        
        if (totalBins > maxSharedMemoryBins) {
            console.warn(
                `HistogramStrategy: Multi-pass radix histogram needs ${totalBins} bins ` +
                `(${numPasses} passes × ${bins.radix} radix), exceeds shared memory limit (${maxSharedMemoryBins}). ` +
                `Will use global atomics (slower).`
            );
        }
    }
}
...
function planBuffers(numBins, inputSize, bins, numPasses) {
    const buffers = {
        output: {
            size: numBins * 4,
...
export function buildHistogramStrategy(userConfig) {
    const { bins, datatype, inputSize, numPasses = 1 } = userConfig;
    ...
    } else if (bins.type === 'radix') {
        validateRadixBins(bins, numPasses);
        strategyType = 'radix';
        numBins = bins.radix;
        const shift = bins.shift ?? 0;
        binningFunction = (value) => radixBinCPU(value, shift, bins.radix);
        generateWGSL = () => generateRadixWGSL(bins.radix, datatype, numPasses, shift);

    } else {
        throw new Error(
            `HistogramStrategy: Unknown bins.type '${bins.type}'. Expected 'even', 'custom', or 'radix'.`
        );
    }

    const totalBins = numBins * numPasses;
    const useSharedMemory = (totalBins <= MAX_SHARED_MEMORY_BINS);
    const dimensions = calculateDimensions(inputSize);

    const buffers = planBuffers(numBins, inputSize, bins, numPasses);
