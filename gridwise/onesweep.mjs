...
export class OneSweepSort extends BaseSort {
  ...
  finalizeRuntimeParameters() {
    ...
    // ✅ Calculate actual uniform buffer size BEFORE offset calculations
    this.uniformBufferSize = this.uniformBufferLength * Uint32Array.BYTES_PER_ELEMENT; // 20 bytes
    ...
    // Debug logging for large buffer allocations
    if (this.inputLength > 2 ** 20) { // Log for inputs > 1M elements
      console.log(`[OneSweep] Input: ${this.inputLength.toLocaleString()} elements (${(this.inputLength * this.keyDatatype.bytesPerElement / 1024 / 1024).toFixed(1)} MB)`);
      console.log(`  - passWorkgroupCount: ${this.passWorkgroupCount.toLocaleString()}`);
      console.log(`  - passHistSize: ${(this.passHistSize / 1024 / 1024).toFixed(1)} MB`);
    }
  }
  compute() {
...
const SortOneSweepFunctionalParams = {
  inputLength: range(18, 25).map((i) => 2 ** i), // 2^18 to 2^24 (256K to 16M elements)
  // inputLength: [2048, 4096],
  // inputLength: range(10, 27).map((i) => 2 ** i),
...
const SortOneSweepSingletonParams = {
  inputLength: range(18, 22).map((i) => 2 ** i),
  // inputLength: [2048, 4096],
  // inputLength: range(10, 27).map((i) => 2 ** i),
  datatype: ["u32"],
  type: ["keysonly"],
